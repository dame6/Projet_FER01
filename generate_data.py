...
class Generate_data():
    ...
    def split_test(self, val_filename= 'val'):
        """
        Helper function to split the validation and train data from general test file.
            params:-
                data_path = path to the folder that contains the test data file
        
        csv_path = self.data_path +"/"+ 'test.csv'
        test = pd.read_csv(csv_path)
        validation_data = pd.DataFrame(test.iloc[:3588,:])
        test_data = pd.DataFrame(test.iloc[3588:,:])
        test_data.to_csv(self.data_path+"/"+'finaltest'+".csv")
        validation_data.to_csv(self.data_path+"/"+'val0'+".csv")
        print("Done splitting the test file into validation & final dddd test file")
        """
        """
        ...
        """
        train_csv_path = self.data_path +"/"+ 'train1.csv'
        train = pd.read_csv(train_csv_path)
        validation_data = pd.DataFrame(train.iloc[:3588,:])
        train_data = pd.DataFrame(train.iloc[3588:,:])
        train_data.to_csv(self.data_path + "/train.csv", index=False, header=True)
        validation_data.to_csv(self.data_path + "/" + val_filename + ".csv", index=False, header=True)
        print("Done splitting the test file into validation & final test dddddddddd file")
